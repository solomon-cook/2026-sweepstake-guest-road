...
import './App.css'
import { generateBalancedAllocation, rankAllocation } from './allocator'
import { SCORE_SNAPSHOT, TEAM_SCORES } from './data'
import type { AllocationResult, PlayerCount } from './types'
...
function App() {
  ...
  const [allocation, setAllocation] = useState<AllocationResult>(() =>
    generateBalancedAllocation(TEAM_SCORES, INITIAL_NAMES, 7),
  )

  useEffect(() => {
    startTransition(() => {
      setAllocation(generateBalancedAllocation(TEAM_SCORES, playerNames, playerCount))
    })
  }, [playerCount, playerNames, shuffleCount])

  const metrics = rankAllocation(allocation)
  const visibleNames = playerNames.slice(0, playerCount)
  const topTeams = TEAM_SCORES.slice(0, 12)

  return (
    <main className="page-shell">
      <section className="hero-panel">
        <div className="hero-copy">
          <p className="eyebrow">2026 World Cup Sweepstake</p>
          <h1>Balanced bundles, not pure luck of the draw.</h1>
          <p className="intro">
            Each team has a fixed pre-tournament score based on a June 9, 2026 World Cup
            winner-odds snapshot. The app keeps rerolling candidate draws until it finds a
            tightly balanced split for 7, 8, or 9 players.
          </p>
        </div>
