...
import { TEAM_SCORES } from './data'

describe('team score data', () => {
  test('contains the full 48-team field exactly once', () => {
    expect(TEAM_SCORES).toHaveLength(48)
    expect(new Set(TEAM_SCORES.map((team) => team.name)).size).toBe(48)
  })

  test('normalizes team scores to the 100-point pool within rounding tolerance', () => {
    const total = TEAM_SCORES.reduce((sum, team) => sum + team.score, 0)
    expect(total).toBeGreaterThan(99.7)
    expect(total).toBeLessThan(100.3)
  })

  test('orders teams consistently by score and rank', () => {
    expect(TEAM_SCORES[0].name).toBe('France')
    expect(TEAM_SCORES[1].name).toBe('Spain')
    expect(TEAM_SCORES[0].rank).toBe(1)
    expect(TEAM_SCORES[TEAM_SCORES.length - 1].rank).toBe(48)
  })
})
...
describe('balanced allocation', () => {
  ...
  test.each([
    ...
    const result = generateBalancedAllocation(TEAM_SCORES, names, playerCount)
  ...
  test('keeps the score spread tight for the 7-player version', () => {
    const result = generateBalancedAllocation(TEAM_SCORES, names, 7)
  ...
  test('falls back to Player n for blank names', () => {
    const result = generateBalancedAllocation(TEAM_SCORES, names, 9)
